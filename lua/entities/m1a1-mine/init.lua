...
function ENT:Initialize()
	self:SetModel( "models/m1a1_mine/m1a1_mine.mdl" );
		...
		if IsValid( self.defusor ) and isnumber( self.defuseTime ) then
			if( self.defusor:KeyDown( IN_USE ) and (self:GetPos() - self.defusor:GetPos()):Length() < 100 )then
				if( CurTime() - self.defuseTime > 2 )then
					...
					local w_mine = ents.Create( "weapon-m1a1-mine" );
					...
					if( self.defusor:HasWeapon( "weapon-m1a1-mine" ) )then
						self.defusor:SelectWeapon( "weapon-m1a1-mine" );
					end;
