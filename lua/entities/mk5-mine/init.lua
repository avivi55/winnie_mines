...
function ENT:Initialize()
	self:SetModel( "models/antitankmine_gs_mkv/antitankmine_gs_mkv.mdl" );
		...
		if IsValid( self.defusor ) and isnumber( self.defuseTime ) then
			if( self.defusor:KeyDown( IN_USE ) and (self:GetPos() - self.defusor:GetPos()):Length() < 100 )then
				if( CurTime() - self.defuseTime > 2 )then
					...
					local w_mine = ents.Create( "weapon-mk5-mine" );
					...
					if( self.defusor:HasWeapon( "weapon-mk5-mine" ) )then
						self.defusor:SelectWeapon( "weapon-mk5-mine" );
					end;
