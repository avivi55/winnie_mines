...
function ENT:Initialize()
	self:SetModel( "models/mle_1939_ap/mle_1939_ap.mdl" );
	...
	self.gamemodeCheck = false--self.gamemode == "militaryrp"
		...
		if IsValid( self.defusor ) and isnumber( self.defuseTime ) then
			if( self.defusor:KeyDown( IN_USE ) and (self:GetPos() - self.defusor:GetPos()):Length() < 100 )then
				if( CurTime() - self.defuseTime > 2 )then
					...
					local w_mine = ents.Create( "weapon-mle1939-mine" );
					...
					if( self.defusor:HasWeapon( "weapon-mle1939-mine" ) )then
						self.defusor:SelectWeapon( "weapon-mle1939-mine" );
					end;
