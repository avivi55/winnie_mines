...
function ENT:Initialize()
	self:SetModel( "models/bfv/gadgets/ger_tellermine42.mdl" );
		...
		if IsValid( self.defusor ) and isnumber( self.defuseTime ) then
			if( self.defusor:KeyDown( IN_USE ) and (self:GetPos() - self.defusor:GetPos()):Length() < 100 )then
				if( CurTime() - self.defuseTime > 2 )then
					...
					local w_mine = ents.Create( "weapon-tel-mine" );
					...
					if( self.defusor:HasWeapon( "weapon-tel-mine" ) )then
						self.defusor:SelectWeapon( "weapon-tel-mine" );
					end;
