...
function ENT:Initialize()
	self:SetModel( "models/shrapnel_mine_mk_ii/shrapnel_mine_mk_ii.mdl" );
		...
		if IsValid( self.defusor ) and isnumber( self.defuseTime ) then
			if( self.defusor:KeyDown( IN_USE ) and (self:GetPos() - self.defusor:GetPos()):Length() < 100 )then
				if( CurTime() - self.defuseTime > 2 )then
					...
					local w_mine = ents.Create( "weapon-mk2-mine" );
					...
					if( self.defusor:HasWeapon( "weapon-mk2-mine" ) )then
						self.defusor:SelectWeapon( "weapon-mk2-mine" );
					end;
